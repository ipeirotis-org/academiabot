...
def compute_metrics(
    ...
    """Compute precision, recall, F1 using one-to-one greedy matching.

    Each predicted item can satisfy at most one ground-truth item (and vice versa)
    to avoid inflating metrics when the same name matches multiple truth entries.
    """
    ...
    for qid in university_qids:
        ...
        for provider in providers:
            ...
            try:
                ...
                # Mark as failed if extraction returned empty after retry exhaustion
                if not names:
                    console.print(f"  [yellow]{provider}: no results (retries exhausted or error)[/yellow]")
        ...
        # Exclude providers that failed (raised exception) or returned empty results
        all_providers = [p for p in provider_names.keys()
                        if p not in failed_providers and provider_names[p]]
...
def print_summary_table(df: pd.DataFrame):
    """Print summary table. Guard against empty dataframe."""
